...
Store non-secrets in `AccountBook` local storage under `lunchflow.*` (`lunchflow.mappings`, `lunchflow.importOnOpen`). Store the API key in the same encrypted data file via `LocalStorage.put("lunchflow.apiKey")` only (same as MD+ Plaid `access_tokens`). Do not use `cacheAuthentication` — MD omits `_authentication` on save unless the file password and “store passwords” are both on. Never a sidecar file or logs. UI: password field, Save key, Remove key, Refresh accounts. Mappings persist on Import and on window close (X / Alt+F4 / Escape / Close), not a dedicated Save mappings button.
