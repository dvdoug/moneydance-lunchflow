...
Shipped locally as **`module_build` 40** (`Version.kt`, `meta_info.dict`, and [CHANGELOG.md](CHANGELOG.md) must stay in lockstep). Phase 3 import is done. Phase 4 polish is in progress.
...
- **Pending set-reconcile only our unconfirmed pending `ParentTxn`s** (`lunchflow:pending:` FITID, `isNew`). Remove vanished holds with `deleteItem()`. Do not delete register txns the user has confirmed. Unique pending→posted match (exact amount, merchant, date within 7 days) updates that `ParentTxn` in place (new posted FITID). Ambiguous / amount-changed → delete the **unconfirmed** pending row and add posted. Unconfirmed pending **Description** gets a `[PENDING] ` prefix **after** `showDownloadedTxns`. Never put that prefix on `OnlineTxn.setName` / `ol.orig-payee` (Moneydance similar-payee scoring is prefix-based). Strip a leftover prefix from `ol.orig-payee` on our FITIDs. See [docs/architecture.md](docs/architecture.md).
