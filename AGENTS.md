...
Shipped locally as **`module_build` 41** (`Version.kt`, `meta_info.dict`, and [CHANGELOG.md](CHANGELOG.md) must stay in lockstep). Phase 3 import is done. Phase 4 polish is in progress.
...
- **Pending set-reconcile our `lunchflow:pending:` register rows.** Unique pending→posted match (exact amount, merchant, date within 7 days) updates that parent in place (posted FITID, clear `lunchflow.pending`, drop `[PENDING] `) **even if the user already confirmed the blue dot**. `deleteItem` vanished holds only when still `isNew`. Never delete a confirmed register txn. Ambiguous / amount-changed: delete only unconfirmed pending, add posted download; confirmed unmatched drops the label but stays for the user to Merge. Reminder/typed rows have no our pending FITID — leave them for Merge. Unconfirmed pending **Description** gets `[PENDING] ` **after** `showDownloadedTxns`. Never put that prefix on `OnlineTxn.setName` / `ol.orig-payee`. See [docs/architecture.md](docs/architecture.md).
