...
Shipped locally as **`module_build` 42** (`Version.kt`, `meta_info.dict`, and [CHANGELOG.md](CHANGELOG.md) must stay in lockstep). Phase 3 import is done. Phase 4 polish is in progress.
...
- **Pending set-reconcile our `lunchflow:pending:` register rows**, including after the user confirmed the blue dot. Unique pending→posted match (exact amount, merchant, date within 7 days) updates that parent in place (posted FITID, clear `lunchflow.pending`, drop `[PENDING] `). If it leaves Lunch Flow pending **without** a unique match (amount changed, ambiguous, vanished): `deleteItem` that parent even if confirmed, then add posted as a download when we have one. Never delete reminder/typed rows or posted `lunchflow:` FITIDs. Never follow a split onto another account’s parent. Unconfirmed pending **Description** gets `[PENDING] ` **after** `showDownloadedTxns`. Never put that prefix on `OnlineTxn.setName` / `ol.orig-payee`. See [docs/architecture.md](docs/architecture.md).
